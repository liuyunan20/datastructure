...
class Solution:
    def numSubarrayProductLessThanK_tle(self, nums: List[int], k: int) -> int:
        # fail with test case [1, 1, 1 ...], k = 100, too many 1s
        def product(arr):
            pro = 1
            for num in arr:
                pro *= num
    ...
    def numSubarrayProductLessThanK_tle2(self, nums: List[int], k: int) -> int:
        ...
        while i < n:
            ...
            while window > 0:
                ...
                if math.prod(nums[i: i + window]) < k:
                    result += window
                    break
                else:
                    window -= 1
    ...
    def numSubarrayProductLessThanK_4(self, nums: List[int], k: int) -> int:
        n = len(nums)
        i = 0
        result = 0
        while i < n:
            j = 0
            prod = math.prod(nums[j: i + 1])
